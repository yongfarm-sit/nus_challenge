...
CREATE TABLE IF NOT EXISTS `tp_acc_erp`.`bills` (
  ...
  PRIMARY KEY (`bill_id`),
  INDEX `fk_billing_vendor` (`vendor_id` ASC) VISIBLE,
  CONSTRAINT `fk_billing_vendor`
...
CREATE TABLE IF NOT EXISTS `tp_acc_erp`.`bill_item` (
  ...
  PRIMARY KEY (`item_id`),
  INDEX `fk_bill` (`bill_id` ASC) VISIBLE,
  CONSTRAINT `fk_bill`
    FOREIGN KEY (`bill_id`)
...
CREATE TABLE IF NOT EXISTS `tp_acc_erp`.`customerinvoice` (
  ...
  CONSTRAINT `customerinvoice_ibfk_1`
...
CREATE TABLE IF NOT EXISTS `tp_acc_erp`.`po_items` (
  ...
  CONSTRAINT `po_items_ibfk_1`
    FOREIGN KEY (`order_id`)
    REFERENCES `tp_acc_erp`.`purchase_order` (`order_id`),
  CONSTRAINT `po_items_ibfk_2`
    FOREIGN KEY (`item_id`)
